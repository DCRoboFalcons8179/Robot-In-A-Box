...
/**
 * Autonomous command that continuously moves the Kraken to a position based on the AprilTag ID seen.
 * If AprilTag ID 1 is seen, move to 1.0 rotations.
 * If AprilTag ID 2 is seen, move to 2.0 rotations.
 * And so on up to ID 15 -> 15.0 rotations.
 * This command continuously monitors for new tags and will move to new positions as different tags are detected.
 */
      ...
      if (bestTarget != null) {
        ...
        if (aprilTagId != lastDetectedTagId) {
          ...
          if (aprilTagId >= 1 && aprilTagId <= 32) {
            ...
            System.out.println("AutoMoveToAprilTagID: Detected NEW AprilTag ID " + aprilTagId + 
                             ", moving to " + targetPosition + " rotations");
            krakenSubsystem.setPosition(targetPosition);
            lastDetectedTagId = aprilTagId;
          } else {
            System.out.println("AutoMoveToAprilTagID: Detected AprilTag ID " + aprilTagId + 
