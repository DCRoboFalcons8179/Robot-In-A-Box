...
public class MoveToPositionIfAprilTagSeen extends Command {
   ...
   */
  public MoveToPositionIfAprilTagSeen(
      VisionSubsystem vision, KrakenSubsystem kraken, int aprilTagId, double position) {
  ...
  public MoveToPositionIfAprilTagSeen(VisionSubsystem vision, KrakenSubsystem kraken) {
    this(
        vision,
        kraken,
        VisionConstants.TARGET_APRILTAG_ID,
        VisionConstants.APRILTAG_TARGET_POSITION);
  }
  ...
  public void execute() {
    ...
    if (visionSubsystem.canSeeAprilTag(targetAprilTagId)) {
      ...
      if (!tagWasSeen) {
        ...
        tagWasSeen = true;
        System.out.println(
            "AprilTag " + targetAprilTagId + " detected! Moving to position " + targetPosition);
      }
    }
  }
